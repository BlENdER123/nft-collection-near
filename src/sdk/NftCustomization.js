...
function NftCustomization() {
	...
	const [posImg, setPosImg] = React.useState({x: 0, y: 0});
	...
	useEffect(() => {
		if (editor) {
			...
			//setPosImg({x: selectedObjects.left, y: selectedObjects.top});
		}
	}, [editor]);
	...
	const _onReady = async (canvas) => {
		await fabric.Image.fromURL(classArr[0].url[curentImages[0]], (img) => {
			img.set({
				left: 0,
				top: 0,
			});
			console.log("nftAreaSize.height", nftAreaSize);
			img.scaleToHeight(nftAreaSize.height);
			img.scaleToWidth(nftAreaSize.width);
			console.log("editor canvas: ", canvas);
			console.log("editor canvas: ", img);
